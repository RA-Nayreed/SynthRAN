...
def _finite_measurement(row: dict[str, Any], outcome: str) -> tuple[float | None, str | None]:
...
def _seed_contrast(
    ...
    *,
    ...
    for arm in required_arms:
        value, reason = _finite_measurement(by_key[(seed, load, arm)], outcome)
...
def analysis(root: Path, *, manifest: dict[str, Any], environment: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    # Main effects by load, outcome, and prespecified contrast.
    for load in loads:
    ...
    # Paired interaction effects: timing penalty at a stronger load minus the
    # corresponding timing penalty at the reference load for the same seed.
