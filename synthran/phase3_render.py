"""Render run-scoped Contiki-NG/Cooja inputs for Phase 3."""
...
from synthran.phase3_runtime import Phase3Error, Phase3Scenario
...
def render_generated_header(scenario: Phase3Scenario) -> str:
    return "\n".join(
        (
            "#ifndef SYNTHRAN_PHASE3_GENERATED_H_",
            "#define SYNTHRAN_PHASE3_GENERATED_H_",
            "",
...
def render_cooja_scenario(scenario: Phase3Scenario) -> str:
    """Render one BR plus exactly ten deterministic MQTT sensors.

    Sensor mote IDs are 1..10 because the firmware maps the Cooja mote ID
    directly to sensor-01..sensor-10.  The border router deliberately uses ID
    250 so it can never be mistaken for a telemetry sensor.
    """

    if scenario.sensor_count != 10:
        raise Phase3Error("Cooja renderer supports exactly 10 sensors")

    # A compact fixed grid keeps every sensor inside the BR's deterministic
    # 75-unit transmit radius while avoiding coincident positions.
    positions = (
        (-30.0, -20.0), (-10.0, -20.0), (10.0, -20.0), (30.0, -20.0),
        (-40.0, 10.0), (-20.0, 10.0), (0.0, 10.0), (20.0, 10.0),
        (40.0, 10.0), (0.0, 40.0),
    )
    sensor_motes = "\n".join(
        _mote(index, x, y)
...
<simconf version=\"2022112801\">
  <simulation>
    <title>SynthRAN Phase 3 {escape(scenario.run_id)}</title>
...
def write_run_inputs(
    ...
    *,
    ...
    """Materialize only run-scoped generated inputs below an ignored run dir."""
    ...
    header = sensor_dir / "phase3-generated.h"
    csc = cooja_dir / "phase3.csc"
    ...
    scenario_json.write_text(
        __import__("json").dumps(scenario.to_dict(), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
        newline="\n",
    )
