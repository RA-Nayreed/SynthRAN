...
def main() -> None:
    ...
    dep['platform'] = choose('Platform', dep['platform'], ('rfsim', 'r2lab', 'physical'))
    dep['ru'] = 'rfsim' if dep['platform'] == 'rfsim' else choose('Radio unit', dep.get('ru', ''))
    ...
    profile_path = Path('deployment/group_vars/all') / f"5g_profile_{dep['profile']}.yaml"
    ...
    for key, label in [('reservation', 'SOP'), ('r2lab_reservation', 'R2Lab')]:
        ...
        settings['enabled'] = choose(f'{label} reservation', str(settings.get('enabled', True)).lower(), ('true', 'false')) == 'true'
        if settings['enabled']:
            settings['duration_minutes'] = int(choose(f'{label} duration in minutes', str(settings.get('duration_minutes', 120))))
