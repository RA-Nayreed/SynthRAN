...
PHASE3_SCHEMA = "synthran/iot-experiment/v1alpha1"
PHASE3_EVIDENCE_SCHEMA = "synthran/iot-evidence/v1alpha1"
...
class Phase3Error(RuntimeError):
    """Raised when the Phase 3 experiment contract is violated."""
...
def sha256_file(path: Path) -> str:
    try:
        ...
        raise Phase3Error(f"unable to hash {path}") from exc
...
class Phase3Scenario:
    ...
    def __post_init__(self) -> None:
        ...
        if self.sensor_count != SENSOR_COUNT:
            raise Phase3Error("initial Phase 3 golden path requires exactly 10 sensors")
        if self.sensor_period_seconds <= 0 or self.sensor_period_seconds > 3600:
            raise Phase3Error("sensor period must be between 1 and 3600 seconds")
        if self.cooja_seed < 0:
            raise Phase3Error("Cooja seed must be non-negative")
        if self.serial_socket_port < 1024 or self.serial_socket_port > 65535:
            raise Phase3Error("serial socket port must be between 1024 and 65535")
        try:
    ...
    *,
    ...
    return "\n".join(
        (
            ...
            "notifications false",
            "restart_timeout 5",
            f"topic {scenario.topic_root}/# out 0",
            "",
        )
    )
...
def render_tunslip_command(
    scenario: Phase3Scenario,
    *,
    contiki_root: Path,
) -> tuple[str, ...]:
    executable = contiki_root / "tools" / "serial-io" / "tunslip6"
    return (
        str(executable),
        "-a",
        "127.0.0.1",
...
class TelemetryEvent:
    ...
    def from_payload(cls, payload: bytes | str, expected_run_id: str) -> "TelemetryEvent":
        ...
        try:
            ...
            raise Phase3Error("telemetry payload is not valid UTF-8 JSON") from exc
        if not isinstance(value, dict):
            raise Phase3Error("telemetry payload must be a JSON object")
        if value.get("schema") != TELEMETRY_SCHEMA:
            raise Phase3Error("telemetry schema is unsupported")
        ...
        if run_id != expected_run_id:
            raise Phase3Error("telemetry run ID does not match the active experiment")
...
def append_jsonl(path: Path, record: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        raise Phase3Error("unable to append JSONL telemetry") from exc
...
def load_jsonl(path: Path, *, expected_run_id: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise Phase3Error("unable to read JSONL telemetry") from exc
    for number, line in enumerate(lines, start=1):
        try:
            value = json.loads(line)
        except json.JSONDecodeError as exc:
            raise Phase3Error(f"JSONL line {number} is invalid") from exc
        if not isinstance(value, dict) or value.get("run_id") != expected_run_id:
            raise Phase3Error(f"JSONL line {number} does not belong to the run")
        if value.get("schema") != TELEMETRY_SCHEMA:
            raise Phase3Error(f"JSONL line {number} has an unsupported schema")
        sensor_id = value.get("sensor_id")
        sequence = value.get("sequence")
        if not isinstance(sensor_id, str) or not SENSOR_RE.fullmatch(sensor_id):
            raise Phase3Error(f"JSONL line {number} has an invalid sensor ID")
        if not isinstance(sequence, int) or isinstance(sequence, bool) or sequence < 1:
            raise Phase3Error(f"JSONL line {number} has an invalid sequence")
...
def write_parquet(records: Sequence[Mapping[str, Any]], destination: Path) -> None:
    ...
    try:
        ...
        raise Phase3Error("PyArrow is required for Parquet derivation") from exc
    ...
    try:
        ...
        raise Phase3Error("unable to write Parquet dataset") from exc
...
def validate_sequence_integrity(
    ...
    *,
    ...
    if minimum_per_sensor < 1:
        raise Phase3Error("minimum_per_sensor must be positive")
    ...
    for record in records:
        ...
        if sensor_id not in by_sensor or not isinstance(sequence, int):
            raise Phase3Error("dataset contains telemetry outside the sensor contract")
...
class Phase3Evidence:
    ...
    def render(self) -> str:
        lines = [f"SynthRAN IoT-to-5G verification ({self.run_id})"]
        for check in self.checks:
            lines.append(
...
def build_offline_data_evidence(
    *,
    scenario: Phase3Scenario,
    scenario_path: Path,
    jsonl_path: Path,
    parquet_path: Path,
    minimum_per_sensor: int = 1,
    extra_checks: Iterable[Phase3Check] = (),
...
def save_phase3_evidence(report: Phase3Evidence, destination: Path) -> None:
