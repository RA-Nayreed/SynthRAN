...
def run_doctor(
    *,
    ...
    try:
        ...
        checks.append(
            R2LabCheck("gateway", False, "strict public-key SSH to Faraday failed")
        )
    ...
    try:
        ...
        checks.append(
            R2LabCheck("lease", False, "no active R2Lab lease could be verified")
        )
        return R2LabDoctorReport(selection, tuple(checks))
    ...
    *,
    ...
    def remote(stage: str, *command: str) -> None:
        ...
        try:
            ...
            log_lines.append(f"{stage}: FAIL - gateway command could not complete")
