...
UE attach, Helm, or Kubernetes scale mutation. Raw modem and gNB output is
reduced immediately to sanitized evidence before it can enter persistent state.
"""
...
def _gateway_command(slice_name: str, *remote: str) -> tuple[str, ...]:
    # Local import avoids a package import cycle while reusing the one approved
    # Faraday identity/host-key boundary from the resource controller.
...
def _qfit_gateway_command(slice_name: str, qfit: str, *remote: str) -> tuple[str, ...]:
    ...
    # The whole inner argv is one remote command string. We intentionally rely
    # on Faraday's existing known-host trust and fail closed if qfit is unknown.
...
def qfit_runtime_probe_commands() -> tuple[tuple[str, ...], ...]:
    """Return the fixed read-only probe set for review/tests."""
...
def _safe_qfit_read(
    *,
    ...
    try:
        result = runner(
            _qfit_gateway_command(slice_name, qfit, *tuple(command)),
            timeout_seconds,
        )
...
def execute_qfit_runtime_probe(
    *,
    ...
    """Observe qfit cell/registration/PDU state without attach or subscriber reads."""
    ...
    # Raw outputs are consumed here and never placed in the returned object.
...
def execute_qfit_management_probe(
    *,
    ...
    """Prove only management reachability from Faraday to the selected qfit."""

    qfit = _validate_qfit(qfit)
    try:
        result = runner(
            _gateway_command(slice_name, "ping", "-c", "1", "-W", "1", qfit),
            timeout_seconds,
        )
    except (RuntimeError, OSError):
        return False
...
def parse_n2_log_state(text: str) -> N2State:
    """Recognize only affirmative current-pod N2/SCTP evidence."""
...
def verify_gnb_n2(
    *,
    ...
    """Read current run-bound Deployment/pod/log state and prove N2 fail-closed."""
    ...
    transport_error = any(
        item is None for item in (namespace, deployment, pods_result)
    ) or (pod_name is not None and logs is None)
...
def execute_physical_runtime_verification(
    *,
    ...
    """Verify gNB/N2 -> qfit runtime -> optional user plane in strict stage order.

    This function is observational. It refreshes R2Lab authority but never powers,
    attaches, scales, or redeploys anything. Each accepted/failed stage is written
    atomically when ``evidence_path`` is provided.
    """
    ...
    gnb_source = (
        "sanitized-gnb-n2:"
        f"bound-{int(gnb.deployment_bound)}:"
        f"one-ready-{int(gnb.singleton_ready)}:n2-{gnb.n2_state.value}"
    )
    if gnb.proven:
        ...
        return PhysicalRuntimeVerificationResult(state, gnb, None, None)
    _persist(state, evidence_path)

    # Recheck the active claim/lease/N300 immediately before observing the UE.
    authority = authorize_physical_start(
        run_id=evidence.run_id,
        slice_name=slice_name,
        run_root=run_root,
    ...
    management_ok = execute_qfit_management_probe(
        slice_name=slice_name,
        qfit=authority.ue,
        runner=r2lab_runner,
    ...
    if management_ok:
        state = state.pass_stage(
            ...
            source="sanitized-qfit-management:reachable",
        )
    else:
        state = state.fail_stage(
            PhysicalAcceptanceStage.UE_MANAGEMENT,
            source="sanitized-qfit-management:unreachable",
        )
        _persist(state, evidence_path)
        return PhysicalRuntimeVerificationResult(state, gnb, None, None)
    _persist(state, evidence_path)

    qfit = execute_qfit_runtime_probe(
        slice_name=slice_name,
        qfit=authority.ue,
        runner=r2lab_runner,
        timeout_seconds=min(timeout_seconds, 30),
    ...
    if state.acceptance.failed_stage is not None:
        return PhysicalRuntimeVerificationResult(state, gnb, qfit, None)
    ...
    if user_plane_peer is not None:
        ...
        user_plane = execute_user_plane_probe(
            peer=user_plane_peer,
            runner=qfit_runner,
    ...
    return PhysicalRuntimeVerificationResult(state, gnb, qfit, user_plane)
