...
def parser():
    ...
    workload = commands.add_parser("workload").add_subparsers(dest="command", required=True); rep = workload.add_parser("replay"); rep.add_argument("--trace", required=True); rep.add_argument("--broker", required=True); rep.add_argument("--port", type=int, default=1883); rep.add_argument("--qos", type=int, default=1); rep.add_argument("--interface"); rep.add_argument("--start-utc"); rep.add_argument("--device"); rep.add_argument("--output", default="publisher.jsonl")
...
def main(argv=None):
    ...
    elif (args.area, args.command) == ("workload", "replay"): replay(args.trace, args.broker, args.port, args.qos, args.interface, args.start_utc, args.output, args.device)
