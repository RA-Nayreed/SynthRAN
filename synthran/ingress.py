...
class CountedTcpIngress:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            listener.close()
            raise IngressError("unable to bind the IPv6 ingress listener") from exc
        self._listener = listener
        self._thread = threading.Thread(
            target=self._serve,
            name="synthran-ingress",
            daemon=True,
        )
        self._thread.start()
    ...
    def stop(self) -> None:
        ...
        if self._error is not None:
            raise IngressError("experiment ingress failed") from self._error
