...
def access(deployment: dict) -> dict[str, str]:
    ...
    identity = os.environ.get('R2LAB_IDENTITY_FILE') or settings.get('identity_file', '')
    return {
        'host': settings.get('host', 'faraday.inria.fr'),
        'username': os.environ.get('R2LAB_USERNAME') or settings.get('username') or deployment.get('r2lab_username', ''),
