...
class TelemetryEvent:
    ...
    def from_payload(
        ...
        try:
            ...
            raise ExperimentError("telemetry payload is not valid UTF-8 JSON") from exc
        if not isinstance(value, dict):
            raise ExperimentError("telemetry payload must be a JSON object")
        if value.get("schema") != TELEMETRY_SCHEMA:
            raise ExperimentError("telemetry schema is unsupported")
        ...
        if run_id != expected_run_id:
            raise ExperimentError("telemetry run ID does not match the active experiment")
        if not isinstance(sensor_id, str) or not SENSOR_RE.fullmatch(sensor_id):
            raise ExperimentError("telemetry sensor ID is outside the 10-sensor contract")
