...
from synthran.workspace.store import profile_path
...
class WorkspaceTests(unittest.TestCase):
    ...
    def test_ids_are_incremental_and_rebuild_uses_consumed_directories(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            third = rebuilt.create_experiment(
                profile="default", project="project", now=NOW + timedelta(minutes=2)
            )
            self.assertEqual(first.experiment_id, "sran-20260817-001")
    ...
    def test_r2lab_access_probe_uses_exact_profile_identity(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(record.provider, "r2lab")
