...
const profileValue = (
  ...
  if (draft.reuseProfile && selected) return `${selected.name} · existing`;
...
export const SetupPanel = ({
  ...
  return (
    <Box flexDirection="column" paddingX={1} paddingY={1}>
      ...
      <Text color={theme.muted}>Choose the controller profile and verify provider access before creating local state.</Text>
      <Box height={1} />

      <Row label="Profile" value={profileValue(profiles, selectedProfile, draft)} focused={focusedIndex === 0} />
      ...
      <Row label="SLICES user" value={username} focused={focusedIndex === 3} />
      ...
      <Box height={1} />
      <Text color={theme.muted}>Type into text rows · ←/→ changes choices · Enter initializes explicitly</Text>
