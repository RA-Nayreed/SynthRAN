...
Provider discovery is available in OBSERVE because it is read-only. Creating a local configuration or recording a provider binding requires an explicit switch to OPERATE. Leaving Configure, reloading, or completing either write returns the workbench to OBSERVE.
