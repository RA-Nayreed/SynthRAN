...
- bounded startup, response-size, protocol, provider-read, and cancellation handling;
...
The control protocol is version 3. Its handshake explicitly allows validated local writes and provider reads while declaring provider mutation unavailable. Earlier contract versions remain frozen in `contracts/`.
...
- press Enter on `Create configuration` to persist a new local experiment;
- press Enter on `Provider experiment` to load SLICES experiments when no provider is bound;
- use ←/→ to select one of the loaded provider experiments;
- press Enter on `Bind provider` to reverify and bind the selected provider experiment locally.
