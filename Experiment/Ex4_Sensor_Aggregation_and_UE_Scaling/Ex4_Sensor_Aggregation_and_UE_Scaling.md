...
Read `synthran/scenario.py`, `synthran/deployment_state.py`, `deploy.sh`, `Experiment/workload/replay.py`, `Experiment/deployment/roles/software_ue_publish/`, `Experiment/deployment/roles/publisher/`, and srsRAN's UE/broker configuration roles. Read the model runner/bridge for source identities.
